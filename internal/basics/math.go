...
// Fast sqrt lookup table (1024 entries) - exact copy from AGG's agg_sqrt_tables.cpp
var gSqrtTable = [1024]uint16{
	0,
...
// Matches AGG's int16u g_sqrt_table[1024] from agg_sqrt_tables.cpp
...
// Matches AGG's fast_sqrt implementation exactly
func FastSqrt(val uint32) uint32 {
	// This code matches the pure C implementation from AGG's agg_math.h
	t := val
	bit := 0
	shift := uint32(11)

	// Emulation of x86 "bsr" instruction - find most significant bit
	if bit = int(t >> 24); bit != 0 {
		bit = int(gElderBitTable[bit]) + 24
	} else if bit = int((t >> 16) & 0xFF); bit != 0 {
		bit = int(gElderBitTable[bit]) + 16
	} else if bit = int((t >> 8) & 0xFF); bit != 0 {
		bit = int(gElderBitTable[bit]) + 8
	} else {
		bit = int(gElderBitTable[t])
	}
