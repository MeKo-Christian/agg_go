...
type RendererPrimitives[BR BaseRenderer, CT interface{}] struct {
	ren       BR  // base renderer
	fillColor CT  // current fill color
	lineColor CT  // current line color
...
func NewRendererPrimitives[BR BaseRenderer, CT interface{}](ren BR) *RendererPrimitives[BR, CT] {
	return &RendererPrimitives[BR, CT]{
		ren:   ren,
		currX: 0,
		currY: 0,
	}
...
func (rp *RendererPrimitives[BR, CT]) Attach(ren BR) {
	rp.ren = ren
}

// Coord converts a floating point coordinate to the subpixel scale used by line_bresenham_interpolator.
func (rp *RendererPrimitives[BR, CT]) Coord(c float64) int {
	return int(c * float64(1<<8)) // subpixel_scale = 1 << 8
}

// FillColor sets the fill color.
func (rp *RendererPrimitives[BR, CT]) FillColor(c CT) {
	rp.fillColor = c
}

// LineColor sets the line color.
func (rp *RendererPrimitives[BR, CT]) LineColor(c CT) {
	rp.lineColor = c
}

// GetFillColor returns the current fill color.
func (rp *RendererPrimitives[BR, CT]) GetFillColor() CT {
	return rp.fillColor
}

// GetLineColor returns the current line color.
func (rp *RendererPrimitives[BR, CT]) GetLineColor() CT {
	return rp.lineColor
}

// Rectangle draws a rectangle outline.
func (rp *RendererPrimitives[BR, CT]) Rectangle(x1, y1, x2, y2 int) {
...
func (rp *RendererPrimitives[BR, CT]) SolidRectangle(x1, y1, x2, y2 int) {
	rp.ren.BlendBar(x1, y1, x2, y2, rp.fillColor, basics.CoverFull)
}

// OutlinedRectangle draws a filled rectangle with an outline.
func (rp *RendererPrimitives[BR, CT]) OutlinedRectangle(x1, y1, x2, y2 int) {
...
func (rp *RendererPrimitives[BR, CT]) Ellipse(x, y, rx, ry int) {
...
func (rp *RendererPrimitives[BR, CT]) SolidEllipse(x, y, rx, ry int) {
	ei := primitives.NewEllipseBresenhamInterpolator(rx, ry)
	dx := 0
	dy := -ry
	dy0 := dy
...
func (rp *RendererPrimitives[BR, CT]) OutlinedEllipse(x, y, rx, ry int) {
	ei := primitives.NewEllipseBresenhamInterpolator(rx, ry)
	dx := 0
	dy := -ry
...
func (rp *RendererPrimitives[BR, CT]) Line(x1, y1, x2, y2 int, last bool) {
...
func (rp *RendererPrimitives[BR, CT]) MoveTo(x, y int) {
	rp.currX = x
	rp.currY = y
}

// LineTo draws a line from the current position to (x, y).
func (rp *RendererPrimitives[BR, CT]) LineTo(x, y int, last bool) {
...
func (rp *RendererPrimitives[BR, CT]) Ren() BR {
	return rp.ren
}
