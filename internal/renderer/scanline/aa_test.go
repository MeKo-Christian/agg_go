...
func TestRendererScanlineAA(t *testing.T) {
	baseRenderer := &MockBaseRenderer[string]{}
	spanAllocator := &MockSpanAllocator[string]{}
	spanGenerator := &MockSpanGenerator[string]{}

	t.Run("creation", func(t *testing.T) {
		renderer := NewRendererScanlineAA[*MockBaseRenderer[string], *MockSpanAllocator[string], *MockSpanGenerator[string], string]()

		if renderer == nil {
			t.Fatal("NewRendererScanlineAA returned nil")
		}
	})

	t.Run("creation with components", func(t *testing.T) {
		renderer := NewRendererScanlineAAWithComponents(baseRenderer, spanAllocator, spanGenerator)
	...
	t.Run("render calls RenderScanlineAA", func(t *testing.T) {
		renderer := NewRendererScanlineAAWithComponents(baseRenderer, spanAllocator, spanGenerator)
		...
		scanline := &MockScanline{
			...
			numSpans: 1,
			spans: []SpanData{
				{X: 5, Len: 3, Covers: []basics.Int8u{255, 200, 150}},
			},
		}

		renderer.Render(scanline)

		// Verify that the underlying RenderScanlineAA function was called
