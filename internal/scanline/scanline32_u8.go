...
// X coordinates must be provided in increasing order.
func (sl *Scanline32U8) AddCells(x, length int, covers []CoverType) {
	x -= sl.minX
	if x < 0 {
		diff := -x
		if diff >= length {
...
// X coordinates must be provided in increasing order.
func (sl *Scanline32U8) AddSpan(x, length int, cover uint) {
	x -= sl.minX
	if x < 0 {
		diff := -x
		if diff >= length {
...
// Scanline interface.
func (sl *Scanline32U8) BeginIterator() ScanlineIterator {
	spans := sl.Begin()
	if len(spans) == 0 {
		return &sliceIter32U8{}
	}
	return &sliceIter32U8{spans: spans}
}
