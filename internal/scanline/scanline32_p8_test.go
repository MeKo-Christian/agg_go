...
func TestScanline32P8_BasicOperations(t *testing.T) {
	sl := NewScanline32P8()
...
func TestScanline32P8_NonConsecutiveCells(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
	...
	// Check each span
	expectedX := []Coord32Type{10, 20, 30}
...
func TestScanline32P8_SolidSpans(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_MergeSolidSpans(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_NoMergeDifferentCoverage(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_AddCells(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_MixedOperations(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_ResetSpans(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_EmptyScanline(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 100)
...
func TestScanline32P8_LargeScanline(t *testing.T) {
	sl := NewScanline32P8()
	sl.Reset(0, 1000)
...
func TestScanline32P8_BoundaryConditions(t *testing.T) {
	sl := NewScanline32P8()
...
// Test 32-bit specific functionality - large coordinates that exceed 16-bit range
func TestScanline32P8_LargeCoordinates(t *testing.T) {
	sl := NewScanline32P8()

	// Test with coordinates that exceed 16-bit range (>65535)
	largeX := 100000
	sl.Reset(largeX-100, largeX+100)

	// Add cells with large coordinates
	sl.AddCell(largeX, 128)
	sl.AddCell(largeX+1, 200)
	sl.AddSpan(largeX+10, 5, 255)

	sl.Finalize(90)

	// Should have 2 spans
	if sl.NumSpans() != 2 {
		t.Errorf("Should have 2 spans with large coordinates, got %d", sl.NumSpans())
	}
	...
	// Check first span (consecutive cells)
	if spans[0].X != Coord32Type(largeX) {
		t.Errorf("First span X should be %d, got %d", largeX, spans[0].X)
	}
	if spans[0].Len != 2 {
		t.Errorf("First span length should be 2, got %d", spans[0].Len)
	}

	// Check second span (solid span)
	if spans[1].X != Coord32Type(largeX+10) {
		t.Errorf("Second span X should be %d, got %d", largeX+10, spans[1].X)
	}
	if spans[1].Len != -5 {
		t.Errorf("Second span length should be -5, got %d", spans[1].Len)
	}
}

// Test maximum coordinate values
func TestScanline32P8_MaxCoordinates(t *testing.T) {
	sl := NewScanline32P8()

	// Test with very large coordinates (near int32 max)
	maxX := int(^uint(0) >> 33) // Large but safe value
	sl.Reset(maxX-10, maxX+10)
	...
	spans := sl.Spans()
	if len(spans) != 1 {
		t.Fatalf("Should have 1 span with max coordinates, got %d", len(spans))
	}

	if spans[0].X != Coord32Type(maxX) {
		t.Errorf("Span X should be %d, got %d", maxX, spans[0].X)
	}
}

// Benchmark comparison between 16-bit and 32-bit scanlines
func BenchmarkScanline32P8_AddCell(b *testing.B) {
	sl := NewScanline32P8()
	sl.Reset(0, b.N)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sl.AddCell(i, uint(i%256))
	}
}

func BenchmarkScanline32P8_AddSpan(b *testing.B) {
	sl := NewScanline32P8()
	sl.Reset(0, b.N*10)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sl.AddSpan(i*10, 5, uint(i%256))
	}
}
