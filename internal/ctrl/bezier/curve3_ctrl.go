...
func (c *Curve3Ctrl) SetCurve(x1, y1, x2, y2, x3, y3 float64) {
	c.poly.SetXn(0, x1)
	c.poly.SetYn(0, y1)
	c.poly.SetXn(1, x2)
	c.poly.SetYn(1, y2)
...
func (c *Curve3Ctrl) Curve() *curves.Curve3 {
	c.updateCurve()
	return c.curve
}

// updateCurve updates the internal curve with current control points.
func (c *Curve3Ctrl) updateCurve() {
	c.curve.Init(
...
func (c *Curve3Ctrl) SetLineColor(clr color.RGBA) {
	c.lineColor = clr
	c.poly.SetLineColor(clr)
}

// LineColor returns the current line color.
func (c *Curve3Ctrl) LineColor() color.RGBA {
	return c.lineColor
}
...
func (c *Curve3Ctrl) NumPaths() uint {
	return 6
}

// Rewind resets the vertex generation state.
func (c *Curve3Ctrl) Rewind(pathID uint) {
	c.idx = pathID
...
func (c *Curve3Ctrl) Vertex() (x, y float64, cmd basics.PathCommand) {
	switch c.idx {
...
func (c *Curve3Ctrl) Color(pathID uint) interface{} {
	return c.lineColor
}
