...
// RasterizerCellsAAStyled implements the main rasterization algorithm with concrete CellStyleAA type.
// This is the styled variant that supports compound rasterization with left/right style identifiers.
// It mirrors RasterizerCellsAASimple but uses CellStyleAA instead of CellAA.
type RasterizerCellsAAStyled struct {
	numBlocks      uint32
	maxBlocks      uint32
	currBlock      uint32
	numCells       uint32
...
// NewRasterizerCellsAAStyled creates a new styled cell-based rasterizer with the specified cell block limit
func NewRasterizerCellsAAStyled(cellBlockLimit uint32) *RasterizerCellsAAStyled {
	r := &RasterizerCellsAAStyled{
		numBlocks:      0,
		maxBlocks:      0,
		currBlock:      0,
		numCells:       0,
...
func (r *RasterizerCellsAAStyled) Reset() {
	r.numCells = 0
	r.currBlock = 0
	r.currCell.Initial()
	r.styleCell.Initial()
...
func (r *RasterizerCellsAAStyled) Style(styleCell CellStyleAA) {
...
func (r *RasterizerCellsAAStyled) MinX() int { return r.minX }

// MinY returns the minimum Y coordinate of the bounding box
func (r *RasterizerCellsAAStyled) MinY() int { return r.minY }

// MaxX returns the maximum X coordinate of the bounding box
func (r *RasterizerCellsAAStyled) MaxX() int { return r.maxX }

// MaxY returns the maximum Y coordinate of the bounding box
func (r *RasterizerCellsAAStyled) MaxY() int { return r.maxY }

// SortCells sorts all cells by Y coordinate and then by X coordinate
func (r *RasterizerCellsAAStyled) SortCells() {
	if r.sorted {
		return
	}
...
func (r *RasterizerCellsAAStyled) TotalCells() uint32 {
	return r.numCells
}

// ScanlineNumCells returns the number of cells for the given scanline Y
func (r *RasterizerCellsAAStyled) ScanlineNumCells(y uint32) uint32 {
...
func (r *RasterizerCellsAAStyled) ScanlineCells(y uint32) []*CellStyleAA {
	if !r.sorted || int(y) < r.minY || int(y) > r.maxY {
		return nil
	}
...
func (r *RasterizerCellsAAStyled) Sorted() bool {
	return r.sorted
}

// setCurrCell sets the current cell coordinates, adding the previous cell if needed
func (r *RasterizerCellsAAStyled) setCurrCell(x, y int) {
...
func (r *RasterizerCellsAAStyled) allocateBlock() {
...
func (r *RasterizerCellsAAStyled) renderLine(x1, y1, x2, y2, dy int) {
	dx := x2 - x1
...
func (r *RasterizerCellsAAStyled) sortCellsByXAndConsolidate() {
	h := r.maxY - r.minY + 1
...
func (r *RasterizerCellsAAStyled) quickSortCellsByX(cells []*CellStyleAA) {
...
func (r *RasterizerCellsAAStyled) insertionSortCellsByX(cells []*CellStyleAA) {
...
func (r *RasterizerCellsAAStyled) partitionCells(cells []*CellStyleAA) int {
...
// Note: For styled cells, we only consolidate cells with the same X AND same styles
func (r *RasterizerCellsAAStyled) consolidateCells(cells []*CellStyleAA) []*CellStyleAA {
	if len(cells) == 0 {
		return cells
	}

	consolidated := make([]*CellStyleAA, 0, len(cells))

	i := 0
	for i < len(cells) {
		currentCell := *cells[i] // Make a copy
		currentX := currentCell.GetX()
		currentLeft := currentCell.Left
		currentRight := currentCell.Right

		// Sum all cells with the same X coordinate AND same styles
		j := i + 1
		for j < len(cells) && cells[j].GetX() == currentX &&
			cells[j].Left == currentLeft && cells[j].Right == currentRight {
...
func (r *RasterizerCellsAAStyled) renderHLine(ey, x1, y1, x2, y2 int) {
	ex1 := x1 >> basics.PolySubpixelShift
	ex2 := x2 >> basics.PolySubpixelShift
