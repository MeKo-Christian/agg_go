...
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendPixel(x, y int, c color.Gray32[CS], cover float32) {
	if InBounds(x, y, pf.Width(), pf.Height()) && c.A > 0.0 {
		pixel := pf.PixPtr(x, y)
		if pixel != nil {
			if blender, ok := any(pf.blender).(Gray32Blender); ok {
...
func (pf *PixFmtAlphaBlendGray32[B, CS]) CopyHline(x1, y, x2 int, c color.Gray32[CS]) {
	if y < 0 || y >= pf.Height() {
		return
	}
	...
	if x1 <= x2 {
		row := pf.RowPtr(y)
		CopyGray32Hline(row, x1, x2-x1+1, c)
	}
}

// BlendHline blends a horizontal line with coverage
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendHline(x1, y, x2 int, c color.Gray32[CS], cover float32) {
	if y < 0 || y >= pf.Height() || c.A == 0.0 {
		return
	}
...
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendVline(x, y1, y2 int, c color.Gray32[CS], cover float32) {
...
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendBar(x1, y1, x2, y2 int, c color.Gray32[CS], cover float32) {
	if c.A == 0.0 {
		return
	}
...
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendSolidHspan(x, y, length int, c color.Gray32[CS], covers []float32) {
	...
	if x1 <= x2 {
		...
		if blender, ok := any(pf.blender).(Gray32Blender); ok {
			...
			for i := 0; i < effectiveLength; i++ {
				...
				if coverIndex < len(covers) && covers[coverIndex] > 0.0 && c.A > 0.0 {
...
func (pf *PixFmtAlphaBlendGray32[B, CS]) BlendSolidVspan(x, y, length int, c color.Gray32[CS], covers []float32) {
	...
	for i, currentY := 0, y1; currentY <= y2; i, currentY = i+1, currentY+1 {
		if coverOffset+i < len(covers) && covers[coverOffset+i] > 0.0 {
...
type (
	PixFmtGray32     = PixFmtAlphaBlendGray32[BlenderGray32Linear, color.Linear]
	PixFmtSGray32    = PixFmtAlphaBlendGray32[BlenderGray32SRGB, color.SRGB]
	PixFmtGray32Pre  = PixFmtAlphaBlendGray32[BlenderGray32PreLinear, color.Linear]
	PixFmtSGray32Pre = PixFmtAlphaBlendGray32[BlenderGray32PreSRGB, color.SRGB]
)

// Constructor functions for concrete types
func NewPixFmtGray32(rbuf *buffer.RenderingBufferF32) *PixFmtGray32 {
	return NewPixFmtAlphaBlendGray32[BlenderGray32Linear, color.Linear](rbuf, BlenderGray32Linear{})
}

func NewPixFmtSGray32(rbuf *buffer.RenderingBufferF32) *PixFmtSGray32 {
	return NewPixFmtAlphaBlendGray32[BlenderGray32SRGB, color.SRGB](rbuf, BlenderGray32SRGB{})
}

func NewPixFmtGray32Pre(rbuf *buffer.RenderingBufferF32) *PixFmtGray32Pre {
	return NewPixFmtAlphaBlendGray32[BlenderGray32PreLinear, color.Linear](rbuf, BlenderGray32PreLinear{})
}

func NewPixFmtSGray32Pre(rbuf *buffer.RenderingBufferF32) *PixFmtSGray32Pre {
	return NewPixFmtAlphaBlendGray32[BlenderGray32PreSRGB, color.SRGB](rbuf, BlenderGray32PreSRGB{})
}
