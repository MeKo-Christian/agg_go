...
func TestBlenderRGBAColorOrders(t *testing.T) {
	testCases := []struct {
		...
		{"RGBA", BlenderRGBA[color.Linear, RGBAOrder]{}, color.OrderRGBA},
		{"ARGB", BlenderRGBA[color.Linear, ARGBOrder]{}, color.OrderARGB},
		{"BGRA", BlenderRGBA[color.Linear, BGRAOrder]{}, color.OrderBGRA},
		{"ABGR", BlenderRGBA[color.Linear, ABGROrder]{}, color.OrderABGR},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			switch bl := tc.blender.(type) {
			case BlenderRGBA[color.Linear, RGBAOrder]:
				bl.Set(pixel, testColor)
			case BlenderRGBA[color.Linear, ARGBOrder]:
				bl.Set(pixel, testColor)
			case BlenderRGBA[color.Linear, BGRAOrder]:
				bl.Set(pixel, testColor)
			case BlenderRGBA[color.Linear, ABGROrder]:
				bl.Set(pixel, testColor)
			}
