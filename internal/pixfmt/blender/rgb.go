...
func (bl BlenderRGBPre[CS, O]) BlendPix(dst []basics.Int8u, r, g, b, alpha, cover basics.Int8u) {
	...
	if ca > 0 {
		// For premultiplied blending, multiply source colors by cover only (not by alpha)
		// The "pre" refers to using prelerp formula, not premultiplying input colors
...
func (bl BlenderRGBPre[CS, O]) BlendPixSimple(dst []basics.Int8u, r, g, b, alpha basics.Int8u) {
	if alpha > 0 {
		order := GetRGBColorOrder[O]()
		// For premultiplied blending, use colors directly with prelerp (matching C++ behavior)
...
type BlenderRGBGamma[CS any, O any, G GammaCorrector] struct {
	gamma G
}

// NewBlenderRGBGamma creates a new gamma blender
func NewBlenderRGBGamma[CS any, O any, G GammaCorrector](gamma G) BlenderRGBGamma[CS, O, G] {
...
func (bl BlenderRGBGamma[CS, O, G]) BlendPix(dst []basics.Int8u, r, g, b, alpha, cover basics.Int8u) {
	...
	if blendAlpha > 0 {
		...
		dr := bl.gamma.Dir(dst[order.R])
		dg := bl.gamma.Dir(dst[order.G])
		db := bl.gamma.Dir(dst[order.B])

		sr := bl.gamma.Dir(r)
		sg := bl.gamma.Dir(g)
		sb := bl.gamma.Dir(b)

		dst[order.R] = bl.gamma.Inv(color.RGBA8Lerp(dr, sr, blendAlpha))
		dst[order.G] = bl.gamma.Inv(color.RGBA8Lerp(dg, sg, blendAlpha))
		dst[order.B] = bl.gamma.Inv(color.RGBA8Lerp(db, sb, blendAlpha))
	}
}
...
type BlenderRGB48Gamma[CS any, O any, G Gamma16Corrector] struct {
	gamma G
}

// NewBlenderRGB48Gamma creates a new gamma blender for 16-bit
func NewBlenderRGB48Gamma[CS any, O any, G Gamma16Corrector](gamma G) BlenderRGB48Gamma[CS, O, G] {
...
func (bl BlenderRGB48Gamma[CS, O, G]) BlendPix(dst []basics.Int16u, r, g, b, alpha, cover basics.Int16u) {
	...
	if blendAlpha > 0 {
		...
		dst[order.R] = bl.gamma.Inv(color.RGB16Lerp(dr, sr, blendAlpha))
		dst[order.G] = bl.gamma.Inv(color.RGB16Lerp(dg, sg, blendAlpha))
		dst[order.B] = bl.gamma.Inv(color.RGB16Lerp(db, sb, blendAlpha))
	}
}
...
type BlenderRGB96Gamma[CS any, O any, G Gamma32Corrector] struct {
	gamma G
}

// NewBlenderRGB96Gamma creates a new gamma blender for 32-bit float
func NewBlenderRGB96Gamma[CS any, O any, G Gamma32Corrector](gamma G) BlenderRGB96Gamma[CS, O, G] {
...
func (bl BlenderRGB96Gamma[CS, O, G]) BlendPix(dst []float32, r, g, b, alpha, cover float32) {
	...
	if blendAlpha > 0 {
		...
		invAlpha := 1.0 - blendAlpha
		dst[order.R] = bl.gamma.Inv(dr*invAlpha + sr*blendAlpha)
		dst[order.G] = bl.gamma.Inv(dg*invAlpha + sg*blendAlpha)
		dst[order.B] = bl.gamma.Inv(db*invAlpha + sb*blendAlpha)
	}
}
