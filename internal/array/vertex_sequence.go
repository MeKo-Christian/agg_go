...
// ----------------------------------------------------------------------------
// VertexDistSequence - Concrete vertex sequence for VertexDist
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// LineAAVertexSequence - Concrete vertex sequence for LineAAVertex
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// VertexCmdSequence - Concrete vertex sequence for VertexDistCmd
// ----------------------------------------------------------------------------

// VertexCmdSequence is a concrete vertex sequence for VertexDistCmd.
// This stores vertices with both distance information and path commands.
type VertexCmdSequence struct {
	storage *PodBVector[VertexDistCmd]
}

// NewVertexCmdSequence creates a new vertex command sequence with default block scale.
...
// NewVertexCmdSequenceWithScale creates a new vertex command sequence with specified block scale.
func NewVertexCmdSequenceWithScale(scale BlockScale) *VertexCmdSequence {
	return &VertexCmdSequence{
		storage: NewPodBVectorWithScale[VertexDistCmd](scale),
	}
}
...
// CalculateDistances calculates and stores distances between consecutive vertices.
func (vs *VertexCmdSequence) CalculateDistances() {
	for i := 0; i < vs.storage.Size()-1; i++ {
		curr := vs.storage.At(i)
		next := vs.storage.At(i + 1)
		curr.CalculateDistance(next)
...
// ModifyAt modifies the vertex at the specified index (bounds-checked).
...
func (vs *VertexCmdSequence) RemoveAt(index int) {
	...
	for i := 0; i < vs.Size(); i++ {
		if i != index {
			newSeq.storage.Add(vs.At(i)) // Use storage.Add to avoid validation during copy
		}
	}

	// Replace current storage with new sequence
	vs.storage = newSeq.storage
}

// ----------------------------------------------------------------------------
// ShortenPath - Utility function for path shortening
// ----------------------------------------------------------------------------
