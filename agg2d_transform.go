...
// updateApproximationScales updates the approximation scales for curve and stroke converters.
// This ensures that curves and strokes render correctly after transformations.
func (agg2d *Agg2D) updateApproximationScales() {
	// Calculate world-to-screen scale factor
	scale := agg2d.worldToScreenScalar(1.0)

	// Update curve converter approximation scale
	if agg2d.convCurve != nil {
		agg2d.convCurve.SetApproximationScale(scale)
	}

	// Update stroke converter approximation scale
...
// TransformStack manages a stack of transformation states.
// This allows saving and restoring transformation contexts.
type TransformStack struct {
	stack []*transform.TransAffine
}

// NewTransformStack creates a new transformation stack.
func NewTransformStack() *TransformStack {
	return &TransformStack{
		stack: make([]*transform.TransAffine, 0, 16), // Pre-allocate for 16 levels
	}
}
...
func (agg2d *Agg2D) PushTransform() {
	if agg2d.transformStack == nil {
		agg2d.transformStack = NewTransformStack()
	}
...
func (agg2d *Agg2D) GetViewportTransform(worldX1, worldY1, worldX2, worldY2,
	screenX1, screenY1, screenX2, screenY2 float64, opt ViewportOption) *transform.TransAffine {
